...
function getColorForRoute(r) {
    if (!routeColors) return "#6b7280";
    if (routeColors[r]) return routeColors[r];
    const baseRoute = String(r).split('|')[0].trim();
    if (routeColors[baseRoute]) return routeColors[baseRoute];
    if (String(r).startsWith("JAK")) return routeColors["JAK"];
    if (String(r).includes("KRL") || String(r).includes("Commuter")) return routeColors["KRL"];
    return "#6b7280";
}
...
export function renderCategories() {
    ...
    container.innerHTML = categories.map(cat => `
        <button onclick="window.filterRoute('${cat.id}')" 
                class="category-btn group bg-white rounded-[1.5rem] p-5 shadow-sm border border-gray-100 hover:border-primary hover:shadow-md transition-all duration-300 flex flex-col items-center justify-center h-36 md:h-44" 
                data-mode="${cat.id}">
            <img src="assets/images/${cat.icon}" alt="${cat.name}" 
                 class="w-16 h-16 md:w-20 md:h-20 object-contain mb-4 transition-transform group-hover:scale-110">
            <span class="text-[13px] md:text-[15px] font-bold text-gray-800 group-hover:text-primary transition-colors font-sans">${cat.name}</span>
        </button>
    `).join('');
}
...
function updateRouteListUI() {
    ...
    let filteredRoutes = appData.routes;
    ...
    container.innerHTML = filteredRoutes.map(route => `
        <div class="route-card bg-white rounded-xl p-4 shadow-sm cursor-pointer border border-gray-100 mb-3" onclick="window.openDetail('${route.id}')">
...
window.openDetail = function(routeId) {
    if (!appData) return;
...
window.toggleDirection = function() {
    ...
    setTimeout(() => {
        ...
        window.switchDirection(currentDirectionIndex);
...
window.switchDirection = function(index) {
...
function renderTimeline(stops) {
    ...
    const renderStopHtml = (stop, idx, isLastOverall = false) => {
        ...
        if (currentRouteDetail && currentRouteDetail.mode === 'mikro') {
            isHalteStop = false;
        } else if (currentRouteDetail && currentRouteDetail.defaultIsHalteStop === false) {
            isHalteStop = stop.isHalteStop === true;
        } else if (stop.isHalteStop === false) {
            isHalteStop = false;
        }
    ...
    window.toggleDropGroup = function(id, btn) {
        document.getElementById(id).classList.toggle('hidden');
        btn.querySelector('.chevron-btn').classList.toggle('rotate-180');
    };
    ...
    const renderToggleNode = (id, count, isAfter, isKrlSeparator = false) => {
        ...
        <div class="relative pb-6 flex items-start font-sans">
            ...
            <div class="ml-3 flex-1 min-w-0 pr-2 md:pr-0 relative z-10">
                <button onclick="window.toggleDropGroup('${id}', this)" class="w-full flex items-center justify-between bg-white hover:bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 transition-all duration-300 shadow-sm focus:outline-none group">
...
export function renderDetail() {
    ...
    if (infoBar) {
        ...
        if (details.opsTable && Array.isArray(details.opsTable)) {
            const tableRows = details.opsTable.map((row, idx) => `
                <tr class="divide-x divide-white/10 ${idx % 2 !== 0 ? 'bg-white/5' : ''}">
                    <td class="py-2.5 px-1 font-bold">${row.time1}</td>
                    <td class="py-2.5 px-1 font-medium ${row.dari1.length > 15 ? 'leading-tight' : ''}">${row.dari1}</td>
                    <td class="py-2.5 px-1 font-bold">${row.time2}</td>
                    <td class="py-2.5 px-1 font-medium ${row.dari2.length > 15 ? 'leading-tight' : ''}">${row.dari2}</td>
                </tr>
            `).join('');
        ...
        let gridClass = details.opsTable && Array.isArray(details.opsTable) ? "grid-cols-2" : "grid-cols-1 sm:grid-cols-3";
        let infoGridHtml = `
            <div class="grid ${gridClass} gap-3 md:gap-4 mt-2 bg-black/10 rounded-xl p-4 border border-white/10 font-sans">
                ...
                ${!details.opsTable || !Array.isArray(details.opsTable) ? opsContainerHtml : ''}
            </div>
            ${details.opsTable && Array.isArray(details.opsTable) ? `<div class="bg-black/10 rounded-xl p-4 border border-white/10 font-sans mt-[-10px]">${opsContainerHtml}</div>` : ''}
        `;
        ...
        if (route.directions && route.directions.length === 1) {
                ...
                <div class="bg-white rounded-2xl shadow-[0_8px_30px_rgb(0,0,0,0.12)] p-2 flex items-center justify-between border border-gray-100">
                    <div class="flex items-center justify-between w-full bg-gray-50 p-1.5 rounded-xl border border-gray-100 gap-1.5">
                        ${(route.directions || []).map((dir, i) => `
                            <button id="btn-dir-${i}" onclick="window.switchDirection(${i})" class="flex-1 py-3 px-2 rounded-xl text-[13px] font-bold text-gray-500 hover:bg-white/50 transition-all duration-300 font-sans">${dir.name || ''}</button>
                        `).join('')}
                    </div>
                </div>
            `;
            ...
            floatingCardContainer.innerHTML = `
                <div class="bg-white rounded-2xl shadow-[0_8px_30px_rgb(0,0,0,0.12)] p-4 md:p-5 flex items-center justify-between border border-gray-100">
                    ...
                    <button id="btn-swap-dir" onclick="window.toggleDirection()" class="w-10 h-10 rounded-xl bg-red-50 hover:bg-red-100 flex items-center justify-center text-red-500 shrink-0 transition-colors shadow-sm border border-red-100 group">
